...
rec {
  ...
  # Creates a lagoon-server instance with default port. The server is killed
  # when this script returns.
  runLagoonServer = attrs@{ schema ? "public", enableS3 ? false }:
    let
      runLagoonServer = writeScript "run-lagoon-server"
        ''
        #!${stdenv.shell}
        source $stdenv/setup
        set -euo pipefail
        export PATH=${lib.makeBinPath [ lagoon-server ] }:$PATH
      ...
      runAll =
        if enableS3
        then withS3Server (withPostgreSQL { inherit schema; } runLagoonServer)
        else withPostgreSQL { inherit schema; } runLagoonServer;
    in writeScript "run-lagoon-server"
      ''
        #!${stdenv.shell}
        source $stdenv/setup
        set -euo pipefail
  ...
  # Creates a lagoon-server instance with default port which will be in scope
  # for the continuation
  withLagoonServer = attrs: act:
    let
      path = lib.makeBinPath [ lagoon-server netcat ];
      script = writeScript "with-lagoon-server"
      ''
        #!${stdenv.shell}
        source $stdenv/setup
        set -euo pipefail
        ...
        ${runLagoonServer attrs} &

        dl_server_pid=$!

        traps lagoon-server "kill $dl_server_pid"

        echo "Waiting for lagoon server to be ready"
        dl_attempts=0
        while ! nc -z localhost 22089; do
          if (( dl_attempts > 20 )); then
            echo "Could not connect to lagoon after $dl_attempts attemps"
            exit 1
          fi
          echo "Lagoon server not accepting connections ($dl_attempts)"
          dl_attempts=$((dl_attempts + 1))
          sleep 1
        done

        echo "Lagoon server is accepting connections"
  ...
  withS3Server = act:
    let
      ...
      ''
        ...
        traps lagoon-server "kill -9 $s3_server_pid"
