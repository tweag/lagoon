...
import Lagoon.Interface.Prog hiding (Login)
import qualified Lagoon.Interface.Prog as P
  ...
  where
    ...
    addFooter h =
      if not (isEmpty (helpBody h)) -- proxy for finding out if --help was used
        then h
        else h { helpFooter = stringChunk "Use lagoon --help for more information." }

showWelcome :: IO ()
showWelcome = putStr $ unlines [
      "Ingest new data sources into the lagoon"
    , ""
    , "EXAMPLE USAGE PATTERNS"
    , ""
    , "  lagoon [-d description] -n proteinatlas/cancer sources/proteinatlas/cancer.csv"
    , ""
    ...
    , ""
    , "  lagoon [-d description] -n embl/genes http://ftp.ebi.ac.uk/pub/genes.rpkm.tsv"
    , ""
    ...
    , "in a file called 'lagoon.yaml' in the current directory. Your database"
...
parseInternalCommand = subparser $ mconcat [
    ...
    , command "get-server-url" $ info (helper <*> parseGetServerURL) $
        progDesc "Show the URL of the Lagoon server"
    ]
...
parseClientConfig = ClientConfig
    <$> optional (strOption $ mconcat [
            long "host"
          , help "lagoon-server host"
          ])
    <*> optional (option auto $ mconcat [
            long "port"
          , help "lagoon-server port"
          ])
