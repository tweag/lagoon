...
#' @param lagoon_host The host of the Lagoon server.
#'        Defaults to "localhost" or the value of the environment
#'        variable LAGOON_HOST if defined.
#' @param lagoon_port The port of the Lagoon server.
...
createConfig <- function( pghost = NULL
  ...
  list( PG_HOST = pghost
      ...
      , LAGOON_HOST = test_env(lagoon_host, "LAGOON_HOST")
      , LAGOON_PORT = test_env(lagoon_port, "LAGOON_PORT")
...
getUsers <- function(config = getDefaultConfig()) {
    dbConn <- src_lagoon(config$LAGOON_HOST, config$LAGOON_PORT);
